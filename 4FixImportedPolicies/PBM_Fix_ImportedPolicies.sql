...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_conditions WHERE [name] = N'SQL Server 2005 or a Later Version')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_conditions WHERE [name] = N'Surface Area Configuration for Database Engine 2005 and 2000 Features')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_conditions WHERE [name] = N'Surface Area Configuration for Database Engine 2008 Features')
...
GO
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Surface Area Configuration for Database Engine 2008 Features')
...
*/
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Surface Area Configuration for Service Broker Endpoints')
...
GO
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Surface Area Configuration for SOAP Endpoints')
...
/*
Fix condition on "Last Successful Backup Date" to account for both FULL and DIFF backups
*/
...
GO
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Last Successful Backup Date')
EXEC msdb.dbo.sp_syspolicy_update_policy @name=N'Last Successful Backup Date', @condition_name=N'Safe Last Full or Diff Backup Date', @execution_mode=0, @schedule_uid=N'00000000-0000-0000-0000-000000000000'
GO
...
*/
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Public Not Granted Server Permissions')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'SQL Server Affinity Mask')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log Cluster Disk Resource Corruption Error')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log Device Driver Control Error')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log Device Not Ready Error')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log Disk Defragmentation')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log Failed I/O Request Error')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log I/O Error During Hard Page Fault Error')
...
IF EXISTS (SELECT * FROM msdb.dbo.syspolicy_policies WHERE [name] = N'Windows Event Log Storage System I/O Timeout Error')
